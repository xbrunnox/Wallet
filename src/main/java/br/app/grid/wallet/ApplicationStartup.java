...
public class ApplicationStartup implements ApplicationListener<ApplicationReadyEvent> {
  ...
  public void onApplicationEvent(ApplicationReadyEvent event) {
    ...
    /*
     * try { BufferedReader br = new BufferedReader(new FileReader("kiwi.csv")); String linha =
     * br.readLine(); linha = br.readLine(); DateTimeFormatter formatter =
     * DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"); while (linha != null) { String[] campos =
     * linha.split(","); Pagamento pagamento = Pagamento.builder() .associado(false) .cpf(campos[5])
     ...
     * .formaDePagamento(null) .idCliente(null) .idLoja("9XismOGxkik60xO") .idPedido(campos[0])
     * .idProduto(null) .ip(campos[7]) .nome(campos[3]) .plataforma("Kiwify") .produto(campos[2])
     * .refPedido(campos[0]) .status(campos[1]) .taxas(Double.valueOf(campos[14]))
     * .telefone(campos[6]) .valor(Double.valueOf(campos[15])) .build(); System.out.println(new
     * ObjectMapper().writeValueAsString(pagamento)); if (pagamento.getStatus().equals("paid")) {
     * pagamentoService.gravar(pagamento); } linha = br.readLine(); } } catch (Exception e) { //
     ...
     * 
     * 
     * // tratarOperacoesAbertas(); List<Trade> trades = tradeService.getList("GRSXPD"); for (Trade
     * trade : trades) { BigDecimal entrada = (trade.getDirecao().equals("C") ?
     * BigDecimal.valueOf(trade.getCompra()) : BigDecimal.valueOf(trade.getVenda())); BigDecimal
     * saida = (trade.getDirecao().equals("V") ? BigDecimal.valueOf(trade.getCompra()) :
     ...
     * .lucro(BigDecimal.valueOf(trade.getResultado())).precoEntrada(entrada). precoSaida(saida)
     * .volume(trade.getVolume()).build();
     * 
     * backtestOperacaoService.gravar(operacao); }
     */
    /*
    List<Assinatura> assinaturas = assinaturaService.getList();
    List<Assinatura> assinaturasAtivas = assinaturaService.getListAtivas();
    Map<String, Assinatura> mapaAssinaturasAtivas = new HashMap<>();

    for (Assinatura assinatura : assinaturasAtivas) {
      System.out.println(assinatura.getConta().getId() + "\t" + assinatura.getConta().getNome()
          + "\t" + assinatura.getConta().getCorretora() + "\t"
          + (assinatura.getEmailPagamento() == null ? "" : assinatura.getEmailPagamento()) + "\t"
          + formatarTelefone(assinatura.getTelefone()) + "\t" + assinatura.getDataVencimento());
      mapaAssinaturasAtivas.put(assinatura.getConta().getNome().toLowerCase(), assinatura);
    }
    System.out.println();
    System.out.println();
    System.out.println();
    ...
    for (Assinatura assinatura : assinaturas) {
      String nome = assinatura.getConta().getNome().toLowerCase();
      if (!mapaAssinaturasAtivas.containsKey(nome)) {
        if (!Strings.isBlank(assinatura.getTelefone())) {
          String telefone = formatarTelefone(assinatura.getTelefone());
          System.out.println(assinatura.getConta().getId() + "\t" + assinatura.getConta().getNome()
              + "\t" + assinatura.getConta().getCorretora() + "\t"
              + (assinatura.getEmailPagamento() == null ? "" : assinatura.getEmailPagamento())
              + "\t" + telefone + "\t" + assinatura.getDataVencimento());
          mapaAssinaturasAtivas.put(nome, assinatura);
        }
      }
    }
    */
  ...
  private void tratarOperacoesAbertas() {
    ...
    for (Operacao operacao : operacoes) {
      if (operacao.getAtivo().getCodigo().equals("WDOM23")) {
        GravarOperacaoRequest operacaoFechamento =
            GravarOperacaoRequest.builder().ativo(operacao.getAtivo().getCodigo())
                .conta(operacao.getConta().getId()).data(LocalDateTime.of(2023, 5, 5, 14, 01, 57))
                .direcao("V").expert(operacao.getExpert().getId()).preco(4961.0).tipo("S")
                .volume(operacao.getVolume()).build();
      ...
      if (operacao.getAtivo().getCodigo().equals("WINM23")) {
        GravarOperacaoRequest operacaoFechamento =
            GravarOperacaoRequest.builder().ativo(operacao.getAtivo().getCodigo())
                .conta(operacao.getConta().getId()).data(LocalDateTime.of(2023, 5, 5, 14, 18, 57))
                .direcao("C").expert(operacao.getExpert().getId()).preco(105775.0).tipo("S")
                .volume(operacao.getVolume()).build();
        operacaoService.save(operacaoFechamento);
        System.out.println("Gravando trade");
      }
