...
public class MetatraderController {
	...
	public String sincronizar(@PathVariable(name = "conta") String conta) {
		ContaHistoricoMT historico = routerService.getHistoricoMetatrader(conta, LocalDate.now(), LocalDate.now());
		if (historico != null && historico.getOperacoes() != null && historico.getOperacoes().size() > 0) {
			...
			for (OperacaoMT operacaoMt : historico.getOperacoes()) {
				GravarOperacaoRequest operacao = GravarOperacaoRequest.builder().ativo(operacaoMt.getAtivo())
						.conta(conta).data(operacaoMt.getHorario())
	...
	public String sincronizar(@PathVariable(name = "conta") String conta, @PathVariable(name = "data") String data) {
	...
	public String market(@PathVariable(name = "conta") String conta, @PathVariable(name = "ativo") String ativo,
			@PathVariable(name = "volume") BigDecimal volume,
